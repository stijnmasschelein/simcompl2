...
test_that("no nan result, is data.frame", {
  ...
  simcompl2::run_simulation(data_params = data_params,
                            test_params = test_params,
                            sim_params = sim_params)
  filenames <- list.files(tmp_dir, pattern="*.RDS", full.names=TRUE)
  ldf <- lapply(filenames, readRDS)
  sim <- do.call(rbind, ldf)
...
context("mixed simulation")

test_that("no nan result, is data.frame", {
  tmp_dir <- "tmp_simcompl"
  unlink(tmp_dir, recursive = TRUE)
  data_params = list(obs = list(list(100, 200)),
                     rate = list(list(0.5, .25)),
